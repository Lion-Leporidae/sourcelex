...
- **多仓库联合搜索** —— 一次查询搜索所有已索引仓库，适合微服务架构
- **API 边界发现** —— 通过 AST 分析自动识别 HTTP 路由注册，发现服务间的 API 接口
...
| **多仓库** | 跨仓库联合搜索 | 一次查询搜索所有已索引仓库，`scope=all` |
| **API 发现** | HTTP 路由自动提取 | 从 Gin、Echo、Flask、FastAPI、Express、Spring 自动识别 API 端点 |
...
# 通过 MCP 工具跨仓库搜索
# search(query="HTTP 中间件", scope="all")
...
索引过程中会自动提取 API 端点（路由注册），支持 Gin、Echo、Flask、FastAPI、Express、Spring 等主流框架。
...
```
            ...
            │              │       │              │
    ┌───────▼──────┐ ┌────▼────┐ ┌▼──────────┐ ┌─▼────────────┐
    │  代码实体     │ │ 调用关系 │ │ 代码分块   │ │  API 端点    │
    │  函数/类/方法 │ │ 调用者 → │ │ 用于嵌入   │ │ GET /api/.. │
    │              │ │ 被调用者 │ │           │ │ POST /api/. │
    └───────┬──────┘ └────┬────┘ └─────┬─────┘ └──────┬───────┘
            │              │           │               │
            │              │  ┌────────▼────────┐      │
...
| POST | `/api/v1/search/multi` | **跨仓库联合搜索**（搜索所有已索引仓库） |
...
| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/api/v1/repos` | 列出所有已索引仓库 |
| POST | `/api/v1/repos/active` | 切换活跃仓库 |
| GET | `/api/v1/repos/active` | 获取当前活跃仓库 |
...
│   │   ├── entity/            #   实体提取（函数/类/方法）
│   │   ├── relation/          #   调用关系提取 + API 端点发现
│   │   └── chunker/           #   代码分块（用于嵌入）
│   ├── cmd/                   # CLI 命令（Cobra）
│   ├── config/                # 配置管理（Viper）
│   ├── git/                   # Git 操作（go-git）
│   ├── logger/                # 结构化日志（Zap）
│   ├── mcp/                   # MCP 服务（Gin）—— 25+ 个端点
│   ├── monitor/               # 资源监控
│   ├── repo/                  # 多仓库注册表 + 用户会话管理
│   ├── store/                 # 存储门面
